        ...
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content" onClick={e => e.stopPropagation()}>
                <div className="modal-header">
                    <h2 className="modal-title">🔔 Give Aplu Push</h2>
                    <button className="modal-close" onClick={onClose}>×</button>
                </div>
                {targetLabel && <p className="text-muted" style={{ marginBottom: '1rem', fontSize: '0.85rem' }}>For: {targetLabel}</p>}
                <form onSubmit={handleSubmit}>
                    <div className="input-group">
                        <label className="input-label">Push Site Name</label>
                        <input className="input" type="text" placeholder="e.g. mysite-push" value={siteName} onChange={e => setSiteName(e.target.value)} required autoFocus />
                    </div>
                    <div className="input-group">
                    ...
                    <div className="input-group">
                        <label className="input-label">Push Time</label>
                        <div className="time-picker-row">
                            <select className="input time-select" value={hour} onChange={e => setHour(e.target.value)}>
                                {Array.from({ length: 12 }, (_, i) => i + 1).map(h => (
                                    <option key={h} value={h.toString()}>{h.toString().padStart(2, '0')}</option>
                                ))}
                            </select>
                            <span className="time-separator">:</span>
                            <select className="input time-select" value={minute} onChange={e => setMinute(e.target.value)}>
                                {['00', '15', '30', '45'].map(m => (
